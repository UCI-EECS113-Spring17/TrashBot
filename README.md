...
## Steps to make haar cascade classifier for shoes
OpenCV -  3.2.0

1. Install OpenCV

2. Put your positive images in the `./positive_images` folder and create a list
of them, from the root of the repository, run this command in your shell:
...
4. Use Perl script `bin/createsamples.pl` to create positive samples by combining each positive image with a random negative image and save them to the `./samples` folder:
...
6. Start training the classifier with `opencv_traincascade`, which comes with
...
7. Wait until the process is finished (which takes a long time — a couple of days probably, depending on the computer you have and how big your images are).

8. Your own classifier `classifier/cascade.xml`
